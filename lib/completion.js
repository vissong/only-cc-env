...
complete -F _only_cc_env ccenv
`;
}

function fishCompletionForCommand(cmd) {
  return [
    `complete -c ${cmd} -f`,
    `complete -c ${cmd} -n '__fish_use_subcommand' -a add -d 'Add a new provider configuration'`,
    `complete -c ${cmd} -n '__fish_use_subcommand' -a edit -d 'Edit an existing provider configuration'`,
    `complete -c ${cmd} -n '__fish_use_subcommand' -a list -d 'List all configured providers'`,
    `complete -c ${cmd} -n '__fish_use_subcommand' -a show -d 'Show provider details'`,
    `complete -c ${cmd} -n '__fish_use_subcommand' -a use -d 'Set the active provider'`,
    `complete -c ${cmd} -n '__fish_use_subcommand' -a remove -d 'Remove a provider configuration'`,
    `complete -c ${cmd} -n '__fish_use_subcommand' -a init -d 'Initialize and configure shell integration'`,
    `complete -c ${cmd} -n '__fish_use_subcommand' -a completion -d 'Generate shell completion script'`,
    `complete -c ${cmd} -n '__fish_seen_subcommand_from use edit show remove' -a '(command only-cc-env __complete 2>/dev/null)'`,
  ].join('\n');
}
...
${fishCompletionForCommand('only-cc-env')}

${fishCompletionForCommand('ccenv')}
`;
}
