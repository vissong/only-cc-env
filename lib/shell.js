  ...
  for (const line of lines) {
    ...
    if (skip) {
      if (shell === 'fish') {
        ...
        if (line.trim().startsWith('source ') || line.trim().startsWith('if ') ||
            line.trim().startsWith('test ') || line.trim().startsWith('function only-cc-env') ||
            line.trim().startsWith('command only-cc-env') || line.trim() === '') {
          continue;
        }
        skip = false;
      } else {
